...
gvldash relies extensively on environment settings which **will not work with
Apache/mod_wsgi setups**. It has been deployed successfully with both
Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'gvldash' environment
...
You can now run the usual Django ``runserver`` command (if you don't start the
server from the ``gvldash`` directory, no services will show up)::
...
The base app will now run as it would with the usual ``manage.py runserver`` but
